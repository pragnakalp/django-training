...
### Step 8: Create Task Update View
...
**Key concepts:**
1. `get_object_or_404(Task, pk=pk, created_by=request.user)` - Ensures user owns the task
2. `TaskForm(request.POST, instance=task)` - Pre-populate form with existing task data
3. Standard POST method for form submission
4. Redirect to task list after successful update

**✅ Task update view is ready!**

---

### Step 9: Create Task Delete View (Soft Delete)
...
**Key concepts:**
1. `get_object_or_404(Task, pk=pk, created_by=request.user, is_deleted=False)` - Ensures user owns the task
2. `task.soft_delete()` - Marks task as deleted (sets `is_deleted=True`)
3. Task remains in database for recovery
4. Standard POST method for form submission
5. Confirmation page shown on GET request
...
**The flow:**
1. User clicks delete (GET request) → Show confirmation page
2. User confirms (POST request) → Soft delete task
...
**✅ Task delete view with soft delete is ready!**

---

### Step 10: Create Toggle Complete View

A simple view to mark tasks as complete/incomplete. **Important:** This must be POST-only to prevent accidental toggles from browser prefetch, crawlers, or malicious links.
...
def task_toggle_complete(request, pk):
    """Toggle task completion status (POST-only for security)"""
...
**Security Note:**
- **POST-only**: Prevents accidental toggles from GET requests (browser prefetch, crawlers, image tags)
- **CSRF protection**: Django automatically validates CSRF token on POST requests
- Template must use a form with `{% csrf_token %}` (shown in templates section)
...
**Note:** In our project, we're using no prefix for tasks (root URL), but you can add a prefix if needed:

```python
# Option 1: No prefix (as used in our project)
# taskmanager/urls.py
urlpatterns = [
    path('admin/', admin.site.urls),
    path('', include('tasks.urls')),  # No prefix - tasks at root
]

# Option 2: With prefix (alternative approach)
# taskmanager/urls.py
urlpatterns = [
    path('admin/', admin.site.urls),
    path('tasks/', include('tasks.urls')),  # With prefix - tasks at /tasks/
]
```

**Important:** Our project uses **Option 1** (no prefix), so:
- Task list is at: `/` (not `/tasks/`)
- Task detail is at: `/task/1/` (not `/tasks/task/1/`)
- Task create is at: `/task/create/` (not `/tasks/task/create/`)
...
👉 **Continue to:** [FBV_Templates_and_Forms.md](FBV_Templates_and_Forms.md)
