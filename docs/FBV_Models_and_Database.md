...
class TaskAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """Optimize queries by using select_related for foreign keys"""
        qs = super().get_queryset(request)
        return qs.select_related('created_by')
```

**Explanation of `get_queryset` method:**
- **Purpose**: Customize the queryset used in the admin list view
- **`super().get_queryset(request)`**: Gets the default queryset (all Task objects)
- **`select_related('created_by')`**: Optimizes database queries by performing a SQL JOIN
- **Why optimize?**: Without `select_related`, Django makes a separate database query for each task's creator (N+1 query problem)
- **Result**: Instead of 100+ queries for 100 tasks, we make just 1 query with a JOIN

**Example:**
```python
# Without select_related (BAD - 101 queries for 100 tasks)
# Query 1: Get all tasks
# Query 2-101: Get creator for each task

# With select_related (GOOD - 1 query for 100 tasks)
# Query 1: Get all tasks with their creators in one JOIN query
```
...
👉 **Continue to:** [FBV_Views_and_URLs.md](FBV_Views_and_URLs.md)
